...
export const RiskAllocation = {
  ...
  async updateMany(props: RiskAllocationType[]): Promise<{ count: number } | null> {
    ...
    const client = createApolloClient();
      ...
      const UPDATE_MANY_RISKALLOCATION = gql`
      mutation updateManyRiskAllocation($data: [RiskAllocationCreateManyInput!]!) {
        updateManyRiskAllocation(data: $data) {
          count
        }
      }`;
  ...
  portfolio: prop.portfolio ? {
    ...
    try {
      const response = await client.mutate({ mutation: UPDATE_MANY_RISKALLOCATION, variables: filteredVariables });
      if (response.errors && response.errors.length > 0) throw new Error(response.errors[0].message);
      if (response && response.data && response.data.updateManyRiskAllocation) {
        return response.data.updateManyRiskAllocation;
      } else {
        return null as any;
      }
    } catch (error) {
      console.error('Error in updateManyRiskAllocation:', error);
      throw error;
    }
  },
