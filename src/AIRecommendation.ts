...
export const AIRecommendation = {
  ...
  async updateMany(props: AIRecommendationType[]): Promise<{ count: number } | null> {
    ...
    const client = createApolloClient();
      ...
      const UPDATE_MANY_AIRECOMMENDATION = gql`
      mutation updateManyAIRecommendation($data: [AIRecommendationCreateManyInput!]!) {
        updateManyAIRecommendation(data: $data) {
          count
        }
      }`;
  ...
  asset: prop.asset ? {
    ...
    try {
      const response = await client.mutate({ mutation: UPDATE_MANY_AIRECOMMENDATION, variables: filteredVariables });
      if (response.errors && response.errors.length > 0) throw new Error(response.errors[0].message);
      if (response && response.data && response.data.updateManyAIRecommendation) {
        return response.data.updateManyAIRecommendation;
      } else {
        return null as any;
      }
    } catch (error) {
      console.error('Error in updateManyAIRecommendation:', error);
      throw error;
    }
  },
