    ...
    : { connectOrCreate: {
        ...
        create: {
          ...
          if (response && response.data && response.data.createOneSession) {
            ...
            return null as unknown as SessionType;
          }
...
? {
 ...
 : { connectOrCreate: prop.user.reviewedWaitlistEntries.map((item) => ({
   ...
   */
  async get(props: SessionType, globalClient?: ApolloClientType<NormalizedCacheObject>, whereInput?: Record<string, unknown>): Promise<SessionType | null> {
    // Maximum number of retries for database connection issues
    const MAX_RETRIES = 3;
    let retryCount = 0;
    let lastError: unknown = null;
   ...
   */
  async findMany(props: SessionType, globalClient?: ApolloClientType<NormalizedCacheObject>, whereInput?: Record<string, unknown>): Promise<SessionType[] | null> {
    // Maximum number of retries for database connection issues
    const MAX_RETRIES = 3;
    let retryCount = 0;
    let lastError: unknown = null;
