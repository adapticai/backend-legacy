...
import { logger } from './utils/logger';
  ...
  source: props.source !== undefined ? props.source : undefined,
          ...
          if (response && response.data && response.data.createOneEquityBar) {
            ...
            return null as unknown as EquityBarType;
          }
   ...
   */
  async get(props: EquityBarType, globalClient?: ApolloClientType<NormalizedCacheObject>, whereInput?: Record<string, unknown>): Promise<EquityBarType | null> {
    // Maximum number of retries for database connection issues
    const MAX_RETRIES = 3;
    let retryCount = 0;
    let lastError: unknown = null;
   ...
   */
  async findMany(props: EquityBarType, globalClient?: ApolloClientType<NormalizedCacheObject>, whereInput?: Record<string, unknown>): Promise<EquityBarType[] | null> {
    // Maximum number of retries for database connection issues
    const MAX_RETRIES = 3;
    let retryCount = 0;
    let lastError: unknown = null;
