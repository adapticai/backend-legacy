...
import { logger } from './utils/logger';
  ...
  originalConfidence: props.originalConfidence !== undefined ? props.originalConfidence : undefined,
  ...
  marketConditions: props.marketConditions !== undefined ? props.marketConditions : undefined,
          ...
          if (response && response.data && response.data.createOneSignalOutcome) {
            ...
            return null as unknown as SignalOutcomeType;
          }
          ...
          if (isConstraintViolation) {
            ...
            logger.error("Non-retryable constraint violation in createOneSignalOutcome", {
          ...
          if (isConnectionError && retryCount < MAX_RETRIES - 1) {
            ...
            logger.warn("Database connection error in createOneSignalOutcome, retrying...", {
              operation: 'createOneSignalOutcome',
              model: 'SignalOutcome',
              attempt: retryCount,
   ...
   */
  async createMany(props: SignalOutcomeType[], globalClient?: ApolloClientType<NormalizedCacheObject>, options?: { skipDuplicates?: boolean }): Promise<{ count: number } | null> {
    // Maximum number of retries for database connection issues
    const MAX_RETRIES = 3;
    let retryCount = 0;
    let lastError: unknown = null;
    ...
    while (retryCount < MAX_RETRIES) {
      try {
        ...
        const CREATE_MANY_SIGNALOUTCOME = gql`
          mutation createManySignalOutcome($data: [SignalOutcomeCreateManyInput!]!, $skipDuplicates: Boolean) {
            createManySignalOutcome(data: $data, skipDuplicates: $skipDuplicates) {
              count
            }
          }`;
  ...
  originalConfidence: prop.originalConfidence !== undefined ? prop.originalConfidence : undefined,
  ...
  marketConditions: prop.marketConditions !== undefined ? prop.marketConditions : undefined,
        ...
        if (isConstraintViolation) {
          const constraintMatch = error.message?.match(/constraint\s+"([^"]+)"/);
          logger.warn("Duplicate key in createManySignalOutcome (expected during overlapping fetches)", {
        ...
        if (isConnectionError && retryCount < MAX_RETRIES - 1) {
          retryCount++;
          const delay = Math.pow(2, retryCount) * 100; // Exponential backoff: 200ms, 400ms, 800ms
          logger.warn("Database connection error in createManySignalOutcome, retrying...", {
            operation: 'createManySignalOutcome',
            model: 'SignalOutcome',
            attempt: retryCount,
            maxRetries: MAX_RETRIES,
  ...
  marketConditions: props.marketConditions !== undefined ? props.marketConditions : undefined,
        ...
        if (isConstraintViolation) {
          const constraintMatch = error.message?.match(/constraint\s+"([^"]+)"/);
          logger.error("Non-retryable constraint violation in updateOneSignalOutcome", {
        ...
        if (isConnectionError && retryCount < MAX_RETRIES - 1) {
          retryCount++;
          const delay = Math.pow(2, retryCount) * 100; // Exponential backoff: 200ms, 400ms, 800ms
          logger.warn("Database connection error in updateOneSignalOutcome, retrying...", {
            operation: 'updateOneSignalOutcome',
            model: 'SignalOutcome',
            attempt: retryCount,
            maxRetries: MAX_RETRIES,
  ...
  marketConditions: props.marketConditions !== undefined ? props.marketConditions : undefined,
        ...
        if (isConstraintViolation) {
          const constraintMatch = error.message?.match(/constraint\s+"([^"]+)"/);
          logger.error("Non-retryable constraint violation in upsertOneSignalOutcome", {
        ...
        if (isConnectionError && retryCount < MAX_RETRIES - 1) {
          retryCount++;
          const delay = Math.pow(2, retryCount) * 100; // Exponential backoff: 200ms, 400ms, 800ms
          logger.warn("Database connection error in upsertOneSignalOutcome, retrying...", {
            operation: 'upsertOneSignalOutcome',
            model: 'SignalOutcome',
            attempt: retryCount,
            maxRetries: MAX_RETRIES,
  ...
  marketConditions: prop.marketConditions !== undefined ? prop.marketConditions : undefined,
        ...
        if (isConstraintViolation) {
          const constraintMatch = error.message?.match(/constraint\s+"([^"]+)"/);
          logger.error("Non-retryable constraint violation in updateManySignalOutcome", {
        ...
        if (isConnectionError && retryCount < MAX_RETRIES - 1) {
          retryCount++;
          const delay = Math.pow(2, retryCount) * 100; // Exponential backoff: 200ms, 400ms, 800ms
          logger.warn("Database connection error in updateManySignalOutcome, retrying...", {
            operation: 'updateManySignalOutcome',
            model: 'SignalOutcome',
            attempt: retryCount,
            maxRetries: MAX_RETRIES,
  ...
  async delete(props: SignalOutcomeType, globalClient?: ApolloClientType<NormalizedCacheObject>): Promise<SignalOutcomeType> {
    ...
    while (retryCount < MAX_RETRIES) {
      try {
        ...
        if (isConstraintViolation) {
          const constraintMatch = error.message?.match(/constraint\s+"([^"]+)"/);
          logger.error("Non-retryable constraint violation in deleteOneSignalOutcome", {
        ...
        if (isConnectionError && retryCount < MAX_RETRIES - 1) {
          retryCount++;
          const delay = Math.pow(2, retryCount) * 100; // Exponential backoff: 200ms, 400ms, 800ms
          logger.warn("Database connection error in deleteOneSignalOutcome, retrying...", {
            operation: 'deleteOneSignalOutcome',
            model: 'SignalOutcome',
            attempt: retryCount,
            maxRetries: MAX_RETRIES,
   ...
   */
  async get(props: SignalOutcomeType, globalClient?: ApolloClientType<NormalizedCacheObject>, whereInput?: Record<string, unknown>): Promise<SignalOutcomeType | null> {
    // Maximum number of retries for database connection issues
    const MAX_RETRIES = 3;
    let retryCount = 0;
    let lastError: unknown = null;
  ...
  symbol: props.symbol !== undefined ? {
    equals: props.symbol 
        ...
        if (isConnectionError && retryCount < MAX_RETRIES - 1) {
          retryCount++;
          const delay = Math.pow(2, retryCount) * 100; // Exponential backoff: 200ms, 400ms, 800ms
          logger.warn("Database connection error in getSignalOutcome, retrying...", {
    ...
    while (retryCount < MAX_RETRIES) {
      try {
        ...
        if (isConnectionError && retryCount < MAX_RETRIES - 1) {
          retryCount++;
          const delay = Math.pow(2, retryCount) * 100; // Exponential backoff: 200ms, 400ms, 800ms
          logger.warn("Database connection error in getAllSignalOutcome, retrying...", {
   ...
   */
  async findMany(props: SignalOutcomeType, globalClient?: ApolloClientType<NormalizedCacheObject>, whereInput?: Record<string, unknown>): Promise<SignalOutcomeType[] | null> {
    // Maximum number of retries for database connection issues
    const MAX_RETRIES = 3;
    let retryCount = 0;
    let lastError: unknown = null;
  ...
  symbol: props.symbol !== undefined ? {
    equals: props.symbol 
        ...
        if (isConnectionError && retryCount < MAX_RETRIES - 1) {
          retryCount++;
          const delay = Math.pow(2, retryCount) * 100; // Exponential backoff: 200ms, 400ms, 800ms
          logger.warn("Database connection error in findManySignalOutcome, retrying...", {
