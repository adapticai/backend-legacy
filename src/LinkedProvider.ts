...
export const LinkedProvider = {
      ...
      try {
        ...
        const variables = {
          data: {
            ...
            providerAccountId:
              props.providerAccountId !== undefined
                ? props.providerAccountId
                : undefined,
            email: props.email !== undefined ? props.email : undefined,
            accessToken:
              props.accessToken !== undefined ? props.accessToken : undefined,
            refreshToken:
              props.refreshToken !== undefined ? props.refreshToken : undefined,
            expiresAt:
                    ...
                    connectOrCreate: {
                      ...
                      create: {
                        ...
                        alpacaAccounts: props.user.alpacaAccounts
                          ? Array.isArray(props.user.alpacaAccounts) &&
                            props.user.alpacaAccounts.length > 0 &&
                            props.user.alpacaAccounts.every(
                              (item: any) =>
                                typeof item === 'object' &&
                                'id' in item &&
                                Object.keys(item).length === 1
                            ...
                            ? {
                                connect: props.user.alpacaAccounts.map(
                                  (item: any) => ({
                                    id: item.id,
                                  })
                                ),
                            ...
                            : {
                                connectOrCreate: props.user.alpacaAccounts.map(
                                  (item: any) => ({
                                    where: {
                                      id:
                                        item.id !== undefined
        ...
        if (
          response &&
          response.data &&
          response.data.createOneLinkedProvider
        ) {
    ...
    while (retryCount < MAX_RETRIES) {
      try {
        ...
        const CREATE_MANY_LINKEDPROVIDER = gql`
          mutation createManyLinkedProvider(
            $data: [LinkedProviderCreateManyInput!]!
          ) {
        ...
        const variables = {
          data: props.map((prop) => ({
            ...
            providerAccountId:
              prop.providerAccountId !== undefined
                ? prop.providerAccountId
                : undefined,
            email: prop.email !== undefined ? prop.email : undefined,
            accessToken:
              prop.accessToken !== undefined ? prop.accessToken : undefined,
            refreshToken:
              prop.refreshToken !== undefined ? prop.refreshToken : undefined,
            expiresAt:
        ...
        if (
          response &&
          response.data &&
          response.data.createManyLinkedProvider
        ) {
    ...
    while (retryCount < MAX_RETRIES) {
      try {
                      ...
                      update: {
                        ...
                        alpacaAccounts: props.user.alpacaAccounts
                          ? Array.isArray(props.user.alpacaAccounts) &&
                            ...
                            : {
                                upsert: props.user.alpacaAccounts.map(
                                  (item: any) => ({
                                    where: {
                                      id:
                                        item.id !== undefined
        ...
        if (
          response &&
          response.data &&
          response.data.updateOneLinkedProvider
        ) {
    ...
    while (retryCount < MAX_RETRIES) {
      try {
        ...
        if (
          response &&
          response.data &&
          response.data.upsertOneLinkedProvider
        ) {
    ...
    while (retryCount < MAX_RETRIES) {
      try {
        ...
        const UPDATE_MANY_LINKEDPROVIDER = gql`
          mutation updateManyLinkedProvider(
            $data: [LinkedProviderCreateManyInput!]!
          ) {
          ...
          data: {
            ...
            email:
              prop.email !== undefined
                ...
                : undefined,
            accessToken:
              prop.accessToken !== undefined
                ? {
                    set: prop.accessToken,
                  }
                : undefined,
            refreshToken:
              prop.refreshToken !== undefined
                ? {
                    ...
                    upsert: {
                      ...
                      update: {
                        ...
                        alpacaAccounts: prop.user.alpacaAccounts
                          ? Array.isArray(prop.user.alpacaAccounts) &&
                            prop.user.alpacaAccounts.length > 0 &&
                            prop.user.alpacaAccounts.every(
                              (item: any) =>
                                typeof item === 'object' &&
                                ('id' in item || 'symbol' in item) &&
                                Object.keys(item).length === 1
                            ...
                            ? {
                                connect: prop.user.alpacaAccounts.map(
                                  (item: any) => ({
                                    id: item.id,
                                  })
                                ),
                            ...
                            : {
                                upsert: prop.user.alpacaAccounts.map(
                                  (item: any) => ({
                                    where: {
                                      id:
                                        item.id !== undefined
                      ...
                      create: {
                        ...
                        alpacaAccounts: prop.user.alpacaAccounts
                          ? Array.isArray(prop.user.alpacaAccounts) &&
                            ...
                            : {
                                connectOrCreate: prop.user.alpacaAccounts.map(
                                  (item: any) => ({
                                    where: {
                                      id:
                                        item.id !== undefined
        ...
        if (
          response &&
          response.data &&
          response.data.updateManyLinkedProvider
        ) {
    ...
    while (retryCount < MAX_RETRIES) {
      try {
        ...
        const DELETE_ONE_LINKEDPROVIDER = gql`
          mutation deleteOneLinkedProvider(
            $where: LinkedProviderWhereUniqueInput!
          ) {
        ...
        if (
          response &&
          response.data &&
          response.data.deleteOneLinkedProvider
        ) {
  ...
  async get(
    ...
    globalClient?: ApolloClientType<NormalizedCacheObject>,
    whereInput?: any
  ): Promise<LinkedProviderType | null> {
    // Maximum number of retries for database connection issues
    const MAX_RETRIES = 3;
    let retryCount = 0;
    let lastError: any = null;
