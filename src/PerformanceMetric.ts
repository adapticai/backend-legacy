...
export const PerformanceMetric = {
  ...
  async updateMany(props: PerformanceMetricType[]): Promise<{ count: number } | null> {
    ...
    const client = createApolloClient();
      ...
      const UPDATE_MANY_PERFORMANCEMETRIC = gql`
      mutation updateManyPerformanceMetric($data: [PerformanceMetricCreateManyInput!]!) {
        updateManyPerformanceMetric(data: $data) {
          count
        }
      }`;
  ...
  portfolio: prop.portfolio ? {
    ...
    try {
      const response = await client.mutate({ mutation: UPDATE_MANY_PERFORMANCEMETRIC, variables: filteredVariables });
      if (response.errors && response.errors.length > 0) throw new Error(response.errors[0].message);
      if (response && response.data && response.data.updateManyPerformanceMetric) {
        return response.data.updateManyPerformanceMetric;
      } else {
        return null as any;
      }
    } catch (error) {
      console.error('Error in updateManyPerformanceMetric:', error);
      throw error;
    }
  },
