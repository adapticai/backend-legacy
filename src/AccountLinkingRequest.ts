    ...
    : { connectOrCreate: {
        ...
        create: {
          ...
          if (response && response.data && response.data.createOneAccountLinkingRequest) {
            ...
            return null as unknown as AccountLinkingRequestType;
          }
...
? {
 ...
 : { connectOrCreate: prop.user.reviewedWaitlistEntries.map((item) => ({
   ...
   */
  async get(props: AccountLinkingRequestType, globalClient?: ApolloClientType<NormalizedCacheObject>, whereInput?: Record<string, unknown>): Promise<AccountLinkingRequestType | null> {
    // Maximum number of retries for database connection issues
    const MAX_RETRIES = 3;
    let retryCount = 0;
    let lastError: unknown = null;
   ...
   */
  async findMany(props: AccountLinkingRequestType, globalClient?: ApolloClientType<NormalizedCacheObject>, whereInput?: Record<string, unknown>): Promise<AccountLinkingRequestType[] | null> {
    // Maximum number of retries for database connection issues
    const MAX_RETRIES = 3;
    let retryCount = 0;
    let lastError: unknown = null;
