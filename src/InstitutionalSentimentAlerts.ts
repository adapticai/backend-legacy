...
import { logger } from './utils/logger';
  ...
  resolvedBy: props.resolvedBy !== undefined ? props.resolvedBy : undefined,
          ...
          if (response && response.data && response.data.createOneInstitutionalSentimentAlerts) {
            ...
            return null as unknown as InstitutionalSentimentAlertsType;
          }
          ...
          if (isConstraintViolation) {
            ...
            logger.error("Non-retryable constraint violation in createOneInstitutionalSentimentAlerts", {
          ...
          if (isConnectionError && retryCount < MAX_RETRIES - 1) {
            ...
            logger.warn("Database connection error in createOneInstitutionalSentimentAlerts, retrying...", {
              operation: 'createOneInstitutionalSentimentAlerts',
              model: 'InstitutionalSentimentAlerts',
              attempt: retryCount,
   ...
   */
  async createMany(props: InstitutionalSentimentAlertsType[], globalClient?: ApolloClientType<NormalizedCacheObject>, options?: { skipDuplicates?: boolean }): Promise<{ count: number } | null> {
    // Maximum number of retries for database connection issues
    const MAX_RETRIES = 3;
    let retryCount = 0;
    let lastError: unknown = null;
    ...
    while (retryCount < MAX_RETRIES) {
      try {
        ...
        const CREATE_MANY_INSTITUTIONALSENTIMENTALERTS = gql`
          mutation createManyInstitutionalSentimentAlerts($data: [InstitutionalSentimentAlertsCreateManyInput!]!, $skipDuplicates: Boolean) {
            createManyInstitutionalSentimentAlerts(data: $data, skipDuplicates: $skipDuplicates) {
              count
            }
          }`;
  ...
  resolvedBy: prop.resolvedBy !== undefined ? prop.resolvedBy : undefined,
        ...
        if (isConstraintViolation) {
          const constraintMatch = error.message?.match(/constraint\s+"([^"]+)"/);
          logger.warn("Duplicate key in createManyInstitutionalSentimentAlerts (expected during overlapping fetches)", {
        ...
        if (isConnectionError && retryCount < MAX_RETRIES - 1) {
          retryCount++;
          const delay = Math.pow(2, retryCount) * 100; // Exponential backoff: 200ms, 400ms, 800ms
          logger.warn("Database connection error in createManyInstitutionalSentimentAlerts, retrying...", {
            operation: 'createManyInstitutionalSentimentAlerts',
            model: 'InstitutionalSentimentAlerts',
            attempt: retryCount,
            maxRetries: MAX_RETRIES,
  ...
  updatedAt: props.updatedAt !== undefined ? {
        ...
        if (isConstraintViolation) {
          const constraintMatch = error.message?.match(/constraint\s+"([^"]+)"/);
          logger.error("Non-retryable constraint violation in updateOneInstitutionalSentimentAlerts", {
        ...
        if (isConnectionError && retryCount < MAX_RETRIES - 1) {
          retryCount++;
          const delay = Math.pow(2, retryCount) * 100; // Exponential backoff: 200ms, 400ms, 800ms
          logger.warn("Database connection error in updateOneInstitutionalSentimentAlerts, retrying...", {
            operation: 'updateOneInstitutionalSentimentAlerts',
            model: 'InstitutionalSentimentAlerts',
            attempt: retryCount,
            maxRetries: MAX_RETRIES,
  ...
  resolvedBy: props.resolvedBy !== undefined ? {
        ...
        if (isConstraintViolation) {
          const constraintMatch = error.message?.match(/constraint\s+"([^"]+)"/);
          logger.error("Non-retryable constraint violation in upsertOneInstitutionalSentimentAlerts", {
        ...
        if (isConnectionError && retryCount < MAX_RETRIES - 1) {
          retryCount++;
          const delay = Math.pow(2, retryCount) * 100; // Exponential backoff: 200ms, 400ms, 800ms
          logger.warn("Database connection error in upsertOneInstitutionalSentimentAlerts, retrying...", {
            operation: 'upsertOneInstitutionalSentimentAlerts',
            model: 'InstitutionalSentimentAlerts',
            attempt: retryCount,
            maxRetries: MAX_RETRIES,
  ...
  updatedAt: prop.updatedAt !== undefined ? {
        ...
        if (isConstraintViolation) {
          const constraintMatch = error.message?.match(/constraint\s+"([^"]+)"/);
          logger.error("Non-retryable constraint violation in updateManyInstitutionalSentimentAlerts", {
        ...
        if (isConnectionError && retryCount < MAX_RETRIES - 1) {
          retryCount++;
          const delay = Math.pow(2, retryCount) * 100; // Exponential backoff: 200ms, 400ms, 800ms
          logger.warn("Database connection error in updateManyInstitutionalSentimentAlerts, retrying...", {
            operation: 'updateManyInstitutionalSentimentAlerts',
            model: 'InstitutionalSentimentAlerts',
            attempt: retryCount,
            maxRetries: MAX_RETRIES,
  ...
  async delete(props: InstitutionalSentimentAlertsType, globalClient?: ApolloClientType<NormalizedCacheObject>): Promise<InstitutionalSentimentAlertsType> {
    ...
    while (retryCount < MAX_RETRIES) {
      try {
        ...
        if (isConstraintViolation) {
          const constraintMatch = error.message?.match(/constraint\s+"([^"]+)"/);
          logger.error("Non-retryable constraint violation in deleteOneInstitutionalSentimentAlerts", {
        ...
        if (isConnectionError && retryCount < MAX_RETRIES - 1) {
          retryCount++;
          const delay = Math.pow(2, retryCount) * 100; // Exponential backoff: 200ms, 400ms, 800ms
          logger.warn("Database connection error in deleteOneInstitutionalSentimentAlerts, retrying...", {
            operation: 'deleteOneInstitutionalSentimentAlerts',
            model: 'InstitutionalSentimentAlerts',
            attempt: retryCount,
            maxRetries: MAX_RETRIES,
   ...
   */
  async get(props: InstitutionalSentimentAlertsType, globalClient?: ApolloClientType<NormalizedCacheObject>, whereInput?: Record<string, unknown>): Promise<InstitutionalSentimentAlertsType | null> {
    // Maximum number of retries for database connection issues
    const MAX_RETRIES = 3;
    let retryCount = 0;
    let lastError: unknown = null;
  ...
  type: props.type !== undefined ? {
    equals: props.type 
        ...
        if (isConnectionError && retryCount < MAX_RETRIES - 1) {
          retryCount++;
          const delay = Math.pow(2, retryCount) * 100; // Exponential backoff: 200ms, 400ms, 800ms
          logger.warn("Database connection error in getInstitutionalSentimentAlerts, retrying...", {
    ...
    while (retryCount < MAX_RETRIES) {
      try {
        ...
        if (isConnectionError && retryCount < MAX_RETRIES - 1) {
          retryCount++;
          const delay = Math.pow(2, retryCount) * 100; // Exponential backoff: 200ms, 400ms, 800ms
          logger.warn("Database connection error in getAllInstitutionalSentimentAlerts, retrying...", {
   ...
   */
  async findMany(props: InstitutionalSentimentAlertsType, globalClient?: ApolloClientType<NormalizedCacheObject>, whereInput?: Record<string, unknown>): Promise<InstitutionalSentimentAlertsType[] | null> {
    // Maximum number of retries for database connection issues
    const MAX_RETRIES = 3;
    let retryCount = 0;
    let lastError: unknown = null;
  ...
  type: props.type !== undefined ? {
    equals: props.type 
        ...
        if (isConnectionError && retryCount < MAX_RETRIES - 1) {
          retryCount++;
          const delay = Math.pow(2, retryCount) * 100; // Exponential backoff: 200ms, 400ms, 800ms
          logger.warn("Database connection error in findManyInstitutionalSentimentAlerts, retrying...", {
