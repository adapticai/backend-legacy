...
import { logger } from './utils/logger';
  ...
  export const SyncEvent = {
     ...
     * @returns The created SyncEvent or null.
     */
    async create(props: SyncEventType, globalClient?: ApolloClientType<NormalizedCacheObject>): Promise<SyncEventType> {
  ...
  metadata: props.metadata !== undefined ? props.metadata : undefined,
          ...
          if (response && response.data && response.data.createOneSyncEvent) {
            ...
            return null as unknown as SyncEventType;
          }
          ...
          if (isConstraintViolation) {
            ...
            logger.error("Non-retryable constraint violation in createOneSyncEvent", {
          ...
          if (isConnectionError && retryCount < MAX_RETRIES - 1) {
            ...
            logger.warn("Database connection error in createOneSyncEvent, retrying...", {
              operation: 'createOneSyncEvent',
              model: 'SyncEvent',
              attempt: retryCount,
   ...
   */
  async createMany(props: SyncEventType[], globalClient?: ApolloClientType<NormalizedCacheObject>, options?: { skipDuplicates?: boolean }): Promise<{ count: number } | null> {
    // Maximum number of retries for database connection issues
    const MAX_RETRIES = 3;
    let retryCount = 0;
    let lastError: unknown = null;
    ...
    while (retryCount < MAX_RETRIES) {
      try {
        ...
        const CREATE_MANY_SYNCEVENT = gql`
          mutation createManySyncEvent($data: [SyncEventCreateManyInput!]!, $skipDuplicates: Boolean) {
            createManySyncEvent(data: $data, skipDuplicates: $skipDuplicates) {
              count
            }
          }`;
  ...
  direction: prop.direction !== undefined ? prop.direction : undefined,
  ...
  metadata: prop.metadata !== undefined ? prop.metadata : undefined,
        ...
        if (isConstraintViolation) {
          const constraintMatch = error.message?.match(/constraint\s+"([^"]+)"/);
          logger.warn("Duplicate key in createManySyncEvent (expected during overlapping fetches)", {
        ...
        if (isConnectionError && retryCount < MAX_RETRIES - 1) {
          retryCount++;
          const delay = Math.pow(2, retryCount) * 100; // Exponential backoff: 200ms, 400ms, 800ms
          logger.warn("Database connection error in createManySyncEvent, retrying...", {
            operation: 'createManySyncEvent',
            model: 'SyncEvent',
            attempt: retryCount,
            maxRetries: MAX_RETRIES,
   ...
   */
  async update(props: SyncEventType, globalClient?: ApolloClientType<NormalizedCacheObject>): Promise<SyncEventType> {
    // Maximum number of retries for database connection issues
    const MAX_RETRIES = 3;
    let retryCount = 0;
    let lastError: unknown = null;
  ...
  metadata: props.metadata !== undefined ? props.metadata : undefined,
        ...
        if (isConstraintViolation) {
          const constraintMatch = error.message?.match(/constraint\s+"([^"]+)"/);
          logger.error("Non-retryable constraint violation in updateOneSyncEvent", {
        ...
        if (isConnectionError && retryCount < MAX_RETRIES - 1) {
          retryCount++;
          const delay = Math.pow(2, retryCount) * 100; // Exponential backoff: 200ms, 400ms, 800ms
          logger.warn("Database connection error in updateOneSyncEvent, retrying...", {
            operation: 'updateOneSyncEvent',
            model: 'SyncEvent',
            attempt: retryCount,
            maxRetries: MAX_RETRIES,
   ...
   * @returns The updated SyncEvent or null.
   */
  async upsert(props: SyncEventType, globalClient?: ApolloClientType<NormalizedCacheObject>): Promise<SyncEventType> {
    // Maximum number of retries for database connection issues
    const MAX_RETRIES = 3;
    let retryCount = 0;
    let lastError: unknown = null;
  ...
  metadata: props.metadata !== undefined ? props.metadata : undefined,
        ...
        if (isConstraintViolation) {
          const constraintMatch = error.message?.match(/constraint\s+"([^"]+)"/);
          logger.error("Non-retryable constraint violation in upsertOneSyncEvent", {
        ...
        if (isConnectionError && retryCount < MAX_RETRIES - 1) {
          retryCount++;
          const delay = Math.pow(2, retryCount) * 100; // Exponential backoff: 200ms, 400ms, 800ms
          logger.warn("Database connection error in upsertOneSyncEvent, retrying...", {
            operation: 'upsertOneSyncEvent',
            model: 'SyncEvent',
            attempt: retryCount,
            maxRetries: MAX_RETRIES,
   ...
   * @returns The count of created records or null.
   */
  async updateMany(props: SyncEventType[], globalClient?: ApolloClientType<NormalizedCacheObject>): Promise<{ count: number } | null> {
    // Maximum number of retries for database connection issues
    const MAX_RETRIES = 3;
    let retryCount = 0;
    let lastError: unknown = null;
  ...
  duration: prop.duration !== undefined ? {
            set: prop.duration 
           } : undefined,
  success: prop.success !== undefined ? {
            set: prop.success 
           } : undefined,
  ...
  metadata: prop.metadata !== undefined ? prop.metadata : undefined,
        ...
        if (isConstraintViolation) {
          const constraintMatch = error.message?.match(/constraint\s+"([^"]+)"/);
          logger.error("Non-retryable constraint violation in updateManySyncEvent", {
        ...
        if (isConnectionError && retryCount < MAX_RETRIES - 1) {
          retryCount++;
          const delay = Math.pow(2, retryCount) * 100; // Exponential backoff: 200ms, 400ms, 800ms
          logger.warn("Database connection error in updateManySyncEvent, retrying...", {
            operation: 'updateManySyncEvent',
            model: 'SyncEvent',
            attempt: retryCount,
            maxRetries: MAX_RETRIES,
   ...
   */
  async delete(props: SyncEventType, globalClient?: ApolloClientType<NormalizedCacheObject>): Promise<SyncEventType> {
    // Maximum number of retries for database connection issues
    const MAX_RETRIES = 3;
    let retryCount = 0;
    let lastError: unknown = null;
    ...
    while (retryCount < MAX_RETRIES) {
      try {
        ...
        if (isConstraintViolation) {
          const constraintMatch = error.message?.match(/constraint\s+"([^"]+)"/);
          logger.error("Non-retryable constraint violation in deleteOneSyncEvent", {
        ...
        if (isConnectionError && retryCount < MAX_RETRIES - 1) {
          retryCount++;
          const delay = Math.pow(2, retryCount) * 100; // Exponential backoff: 200ms, 400ms, 800ms
          logger.warn("Database connection error in deleteOneSyncEvent, retrying...", {
            operation: 'deleteOneSyncEvent',
            model: 'SyncEvent',
            attempt: retryCount,
            maxRetries: MAX_RETRIES,
   ...
   */
  async get(props: SyncEventType, globalClient?: ApolloClientType<NormalizedCacheObject>, whereInput?: Record<string, unknown>): Promise<SyncEventType | null> {
    // Maximum number of retries for database connection issues
    const MAX_RETRIES = 3;
    let retryCount = 0;
    let lastError: unknown = null;
  ...
  entityId: props.entityId !== undefined ? {
    equals: props.entityId 
        ...
        if (isConnectionError && retryCount < MAX_RETRIES - 1) {
          retryCount++;
          const delay = Math.pow(2, retryCount) * 100; // Exponential backoff: 200ms, 400ms, 800ms
          logger.warn("Database connection error in getSyncEvent, retrying...", {
   ...
   */
  async getAll(globalClient?: ApolloClientType<NormalizedCacheObject>): Promise<SyncEventType[] | null> {
    // Maximum number of retries for database connection issues
    const MAX_RETRIES = 3;
    let retryCount = 0;
    let lastError: unknown = null;
    ...
    while (retryCount < MAX_RETRIES) {
      try {
        ...
        if (isConnectionError && retryCount < MAX_RETRIES - 1) {
          retryCount++;
          const delay = Math.pow(2, retryCount) * 100; // Exponential backoff: 200ms, 400ms, 800ms
          logger.warn("Database connection error in getAllSyncEvent, retrying...", {
   ...
   */
  async findMany(props: SyncEventType, globalClient?: ApolloClientType<NormalizedCacheObject>, whereInput?: Record<string, unknown>): Promise<SyncEventType[] | null> {
    // Maximum number of retries for database connection issues
    const MAX_RETRIES = 3;
    let retryCount = 0;
    let lastError: unknown = null;
  ...
  entityId: props.entityId !== undefined ? {
    equals: props.entityId 
        ...
        if (isConnectionError && retryCount < MAX_RETRIES - 1) {
          retryCount++;
          const delay = Math.pow(2, retryCount) * 100; // Exponential backoff: 200ms, 400ms, 800ms
          logger.warn("Database connection error in findManySyncEvent, retrying...", {
