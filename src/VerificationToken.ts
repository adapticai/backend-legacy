...
import { logger } from './utils/logger';
  ...
  expires: props.expires !== undefined ? props.expires : undefined,
          ...
          if (response && response.data && response.data.createOneVerificationToken) {
            ...
            return null as unknown as VerificationTokenType;
          }
   ...
   */
  async get(props: VerificationTokenType, globalClient?: ApolloClientType<NormalizedCacheObject>, whereInput?: Record<string, unknown>): Promise<VerificationTokenType | null> {
    // Maximum number of retries for database connection issues
    const MAX_RETRIES = 3;
    let retryCount = 0;
    let lastError: unknown = null;
   ...
   */
  async findMany(props: VerificationTokenType, globalClient?: ApolloClientType<NormalizedCacheObject>, whereInput?: Record<string, unknown>): Promise<VerificationTokenType[] | null> {
    // Maximum number of retries for database connection issues
    const MAX_RETRIES = 3;
    let retryCount = 0;
    let lastError: unknown = null;
