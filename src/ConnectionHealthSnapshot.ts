...
import { logger } from './utils/logger';
  ...
  metadata: props.metadata !== undefined ? props.metadata : undefined,
          ...
          if (response && response.data && response.data.createOneConnectionHealthSnapshot) {
            ...
            return null as unknown as ConnectionHealthSnapshotType;
          }
  ...
  /**
   * Update a single ConnectionHealthSnapshot record.
   * Enhanced with connection resilience against Prisma connection errors.
  ...
  async update(props: ConnectionHealthSnapshotType, globalClient?: ApolloClientType<NormalizedCacheObject>): Promise<ConnectionHealthSnapshotType> {
    ...
    while (retryCount < MAX_RETRIES) {
      try {
        ...
        const UPDATE_ONE_CONNECTIONHEALTHSNAPSHOT = gql`
          mutation updateOneConnectionHealthSnapshot($data: ConnectionHealthSnapshotUpdateInput!, $where: ConnectionHealthSnapshotWhereUniqueInput!) {
            updateOneConnectionHealthSnapshot(data: $data, where: $where) {
              ${selectionSet}
            }
          }`;
  ...
  /**
   * Upsert a single ConnectionHealthSnapshot record.
   * Enhanced with connection resilience against Prisma connection errors.
   * @param props - Properties to update.
   * @param globalClient - Apollo Client instance.
   * @returns The updated ConnectionHealthSnapshot or null.
  ...
  async upsert(props: ConnectionHealthSnapshotType, globalClient?: ApolloClientType<NormalizedCacheObject>): Promise<ConnectionHealthSnapshotType> {
    ...
    while (retryCount < MAX_RETRIES) {
      try {
        ...
        const UPSERT_ONE_CONNECTIONHEALTHSNAPSHOT = gql`
          mutation upsertOneConnectionHealthSnapshot($where: ConnectionHealthSnapshotWhereUniqueInput!, $create: ConnectionHealthSnapshotCreateInput!, $update: ConnectionHealthSnapshotUpdateInput!) {
            upsertOneConnectionHealthSnapshot(where: $where, create: $create, update: $update) {
              ${selectionSet}
            }
          }`;
  ...
  /**
   * Update multiple ConnectionHealthSnapshot records.
   * Enhanced with connection resilience against Prisma connection errors.
   * @param props - Array of ConnectionHealthSnapshot objects for the updated records.
   * @param globalClient - Apollo Client instance.
   * @returns The count of created records or null.
   */
  ...
  /**
   * Delete a single ConnectionHealthSnapshot record.
   * Enhanced with connection resilience against Prisma connection errors.
  ...
  /**
   * Retrieve a single ConnectionHealthSnapshot record by ID.
   * Enhanced with connection resilience against Prisma connection errors.
   ...
   */
  async get(props: ConnectionHealthSnapshotType, globalClient?: ApolloClientType<NormalizedCacheObject>, whereInput?: Record<string, unknown>): Promise<ConnectionHealthSnapshotType | null> {
    // Maximum number of retries for database connection issues
    const MAX_RETRIES = 3;
    let retryCount = 0;
    let lastError: unknown = null;
    ...
    while (retryCount < MAX_RETRIES) {
      try {
        ...
        const GET_CONNECTIONHEALTHSNAPSHOT = gql`
          query getConnectionHealthSnapshot($where: ConnectionHealthSnapshotWhereUniqueInput!) {
            getConnectionHealthSnapshot(where: $where) {
              ${selectionSet}
            }
          }`;
  ...
  status: props.status !== undefined ? {
    equals: props.status 
        ...
        const response = await client.query({
          query: GET_CONNECTIONHEALTHSNAPSHOT,
          variables: filteredVariables,
  ...
  /**
   * Retrieve all ConnectionHealthSnapshots records.
   * Enhanced with connection resilience against Prisma connection errors.
   * @param globalClient - Apollo Client instance.
  ...
  async getAll(globalClient?: ApolloClientType<NormalizedCacheObject>): Promise<ConnectionHealthSnapshotType[] | null> {
    ...
    while (retryCount < MAX_RETRIES) {
      try {
        ...
        const GET_ALL_CONNECTIONHEALTHSNAPSHOT = gql`
          query getAllConnectionHealthSnapshot {
            connectionHealthSnapshots {
              ${selectionSet}
            }
          }`;

        const response = await client.query({
          query: GET_ALL_CONNECTIONHEALTHSNAPSHOT,
          fetchPolicy: 'network-only', // Force network request to avoid stale cache
        });
  ...
  /**
   * Find multiple ConnectionHealthSnapshot records based on conditions.
   * Enhanced with connection resilience against Prisma connection errors.
   ...
   */
  async findMany(props: ConnectionHealthSnapshotType, globalClient?: ApolloClientType<NormalizedCacheObject>, whereInput?: Record<string, unknown>): Promise<ConnectionHealthSnapshotType[] | null> {
    // Maximum number of retries for database connection issues
    const MAX_RETRIES = 3;
    let retryCount = 0;
    let lastError: unknown = null;
    ...
    while (retryCount < MAX_RETRIES) {
      try {
        ...
        const FIND_MANY_CONNECTIONHEALTHSNAPSHOT = gql`
          query findManyConnectionHealthSnapshot($where: ConnectionHealthSnapshotWhereInput!) {
            connectionHealthSnapshots(where: $where) {
              ${selectionSet}
            }
          }`;
