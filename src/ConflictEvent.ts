...
import { logger } from './utils/logger';
  ...
  export const ConflictEvent = {
     ...
     * @returns The created ConflictEvent or null.
     */
    async create(props: ConflictEventType, globalClient?: ApolloClientType<NormalizedCacheObject>): Promise<ConflictEventType> {
  ...
  resolutionTime: props.resolutionTime !== undefined ? props.resolutionTime : undefined,
          ...
          if (response && response.data && response.data.createOneConflictEvent) {
            ...
            return null as unknown as ConflictEventType;
          }
          ...
          if (isConstraintViolation) {
            ...
            logger.error("Non-retryable constraint violation in createOneConflictEvent", {
          ...
          if (isConnectionError && retryCount < MAX_RETRIES - 1) {
            ...
            logger.warn("Database connection error in createOneConflictEvent, retrying...", {
              operation: 'createOneConflictEvent',
              model: 'ConflictEvent',
              attempt: retryCount,
  ...
  resolutionTime: prop.resolutionTime !== undefined ? prop.resolutionTime : undefined,
        ...
        if (response.errors && response.errors.length > 0) throw new Error(response.errors[0].message);
        if (response && response.data && response.data.createManyConflictEvent) {
        ...
        if (isConnectionError && retryCount < MAX_RETRIES - 1) {
          retryCount++;
          const delay = Math.pow(2, retryCount) * 100; // Exponential backoff: 200ms, 400ms, 800ms
          logger.warn("Database connection error in createManyConflictEvent, retrying...", {
            operation: 'createManyConflictEvent',
            model: 'ConflictEvent',
            attempt: retryCount,
            maxRetries: MAX_RETRIES,
   ...
   */
  async update(props: ConflictEventType, globalClient?: ApolloClientType<NormalizedCacheObject>): Promise<ConflictEventType> {
    // Maximum number of retries for database connection issues
    const MAX_RETRIES = 3;
    let retryCount = 0;
    let lastError: unknown = null;
  ...
  resolutionTime: props.resolutionTime !== undefined ? {
        ...
        if (isConstraintViolation) {
          const constraintMatch = error.message?.match(/constraint\s+"([^"]+)"/);
          logger.error("Non-retryable constraint violation in updateOneConflictEvent", {
        ...
        if (isConnectionError && retryCount < MAX_RETRIES - 1) {
          retryCount++;
          const delay = Math.pow(2, retryCount) * 100; // Exponential backoff: 200ms, 400ms, 800ms
          logger.warn("Database connection error in updateOneConflictEvent, retrying...", {
            operation: 'updateOneConflictEvent',
            model: 'ConflictEvent',
            attempt: retryCount,
            maxRetries: MAX_RETRIES,
   ...
   * @returns The updated ConflictEvent or null.
   */
  async upsert(props: ConflictEventType, globalClient?: ApolloClientType<NormalizedCacheObject>): Promise<ConflictEventType> {
    // Maximum number of retries for database connection issues
    const MAX_RETRIES = 3;
    let retryCount = 0;
    let lastError: unknown = null;
  ...
  resolutionTime: props.resolutionTime !== undefined ? {
        ...
        if (isConstraintViolation) {
          const constraintMatch = error.message?.match(/constraint\s+"([^"]+)"/);
          logger.error("Non-retryable constraint violation in upsertOneConflictEvent", {
        ...
        if (isConnectionError && retryCount < MAX_RETRIES - 1) {
          retryCount++;
          const delay = Math.pow(2, retryCount) * 100; // Exponential backoff: 200ms, 400ms, 800ms
          logger.warn("Database connection error in upsertOneConflictEvent, retrying...", {
            operation: 'upsertOneConflictEvent',
            model: 'ConflictEvent',
            attempt: retryCount,
            maxRetries: MAX_RETRIES,
   ...
   * @returns The count of created records or null.
   */
  async updateMany(props: ConflictEventType[], globalClient?: ApolloClientType<NormalizedCacheObject>): Promise<{ count: number } | null> {
    // Maximum number of retries for database connection issues
    const MAX_RETRIES = 3;
    let retryCount = 0;
    let lastError: unknown = null;
    ...
    while (retryCount < MAX_RETRIES) {
      try {
        ...
        const UPDATE_MANY_CONFLICTEVENT = gql`
          mutation updateManyConflictEvent($data: [ConflictEventCreateManyInput!]!) {
  ...
  resolutionTime: prop.resolutionTime !== undefined ? {
        ...
        if (response.errors && response.errors.length > 0) throw new Error(response.errors[0].message);
        if (response && response.data && response.data.updateManyConflictEvent) {
        ...
        if (isConstraintViolation) {
          const constraintMatch = error.message?.match(/constraint\s+"([^"]+)"/);
          logger.error("Non-retryable constraint violation in updateManyConflictEvent", {
        ...
        if (isConnectionError && retryCount < MAX_RETRIES - 1) {
          retryCount++;
          const delay = Math.pow(2, retryCount) * 100; // Exponential backoff: 200ms, 400ms, 800ms
          logger.warn("Database connection error in updateManyConflictEvent, retrying...", {
            operation: 'updateManyConflictEvent',
            model: 'ConflictEvent',
            attempt: retryCount,
            maxRetries: MAX_RETRIES,
   ...
   */
  async delete(props: ConflictEventType, globalClient?: ApolloClientType<NormalizedCacheObject>): Promise<ConflictEventType> {
    // Maximum number of retries for database connection issues
    const MAX_RETRIES = 3;
    let retryCount = 0;
    let lastError: unknown = null;
    ...
    while (retryCount < MAX_RETRIES) {
      try {
        ...
        const DELETE_ONE_CONFLICTEVENT = gql`
          mutation deleteOneConflictEvent($where: ConflictEventWhereUniqueInput!) {
        ...
        if (isConstraintViolation) {
          const constraintMatch = error.message?.match(/constraint\s+"([^"]+)"/);
          logger.error("Non-retryable constraint violation in deleteOneConflictEvent", {
        ...
        if (isConnectionError && retryCount < MAX_RETRIES - 1) {
          retryCount++;
          const delay = Math.pow(2, retryCount) * 100; // Exponential backoff: 200ms, 400ms, 800ms
          logger.warn("Database connection error in deleteOneConflictEvent, retrying...", {
            operation: 'deleteOneConflictEvent',
            model: 'ConflictEvent',
            attempt: retryCount,
            maxRetries: MAX_RETRIES,
   ...
   */
  async get(props: ConflictEventType, globalClient?: ApolloClientType<NormalizedCacheObject>, whereInput?: Record<string, unknown>): Promise<ConflictEventType | null> {
    // Maximum number of retries for database connection issues
    const MAX_RETRIES = 3;
    let retryCount = 0;
    let lastError: unknown = null;
  ...
  entityId: props.entityId !== undefined ? {
    equals: props.entityId 
        ...
        if (isConnectionError && retryCount < MAX_RETRIES - 1) {
          retryCount++;
          const delay = Math.pow(2, retryCount) * 100; // Exponential backoff: 200ms, 400ms, 800ms
          logger.warn("Database connection error in getConflictEvent, retrying...", {
   ...
   */
  async getAll(globalClient?: ApolloClientType<NormalizedCacheObject>): Promise<ConflictEventType[] | null> {
    // Maximum number of retries for database connection issues
    const MAX_RETRIES = 3;
    let retryCount = 0;
    let lastError: unknown = null;
    ...
    while (retryCount < MAX_RETRIES) {
      try {
        ...
        if (isConnectionError && retryCount < MAX_RETRIES - 1) {
          retryCount++;
          const delay = Math.pow(2, retryCount) * 100; // Exponential backoff: 200ms, 400ms, 800ms
          logger.warn("Database connection error in getAllConflictEvent, retrying...", {
   ...
   */
  async findMany(props: ConflictEventType, globalClient?: ApolloClientType<NormalizedCacheObject>, whereInput?: Record<string, unknown>): Promise<ConflictEventType[] | null> {
    // Maximum number of retries for database connection issues
    const MAX_RETRIES = 3;
    let retryCount = 0;
    let lastError: unknown = null;
  ...
  entityId: props.entityId !== undefined ? {
    equals: props.entityId 
        ...
        if (isConnectionError && retryCount < MAX_RETRIES - 1) {
          retryCount++;
          const delay = Math.pow(2, retryCount) * 100; // Exponential backoff: 200ms, 400ms, 800ms
          logger.warn("Database connection error in findManyConflictEvent, retrying...", {
