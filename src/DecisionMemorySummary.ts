...
import { logger } from './utils/logger';
  ...
  expiresAt: props.expiresAt !== undefined ? props.expiresAt : undefined,
          ...
          if (response && response.data && response.data.createOneDecisionMemorySummary) {
            ...
            return null as unknown as DecisionMemorySummaryType;
          }
   ...
   */
  async get(props: DecisionMemorySummaryType, globalClient?: ApolloClientType<NormalizedCacheObject>, whereInput?: Record<string, unknown>): Promise<DecisionMemorySummaryType | null> {
    // Maximum number of retries for database connection issues
    const MAX_RETRIES = 3;
    let retryCount = 0;
    let lastError: unknown = null;
   ...
   */
  async findMany(props: DecisionMemorySummaryType, globalClient?: ApolloClientType<NormalizedCacheObject>, whereInput?: Record<string, unknown>): Promise<DecisionMemorySummaryType[] | null> {
    // Maximum number of retries for database connection issues
    const MAX_RETRIES = 3;
    let retryCount = 0;
    let lastError: unknown = null;
