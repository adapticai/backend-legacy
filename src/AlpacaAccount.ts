...
import { logger } from './utils/logger';
 ...
 : { connectOrCreate: props.user.reviewedWaitlistEntries.map((item) => ({
  ...
  alerts: props.alerts ? 
    Array.isArray(props.alerts) && props.alerts.length > 0 &&  props.alerts.every((item: unknown) => typeof item === 'object' && item !== null && 'id' in item && Object.keys(item).length === 1) ? {
      ...
      create: {
        ...
        metadata: item.metadata !== undefined ? item.metadata : undefined,
          ...
          if (response && response.data && response.data.createOneAlpacaAccount) {
            ...
            return null as unknown as AlpacaAccountType;
          }
...
? {
 ...
 : { connectOrCreate: props.user.reviewedWaitlistEntries.map((item) => ({
  ...
  alerts: props.alerts ? 
  Array.isArray(props.alerts) && props.alerts.length > 0 && props.alerts.every((item: unknown) => typeof item === 'object' && item !== null && ('id' in item || 'symbol' in item) && Object.keys(item).length === 1) ? {
  connect: props.alerts.map((item) => ({
    id: item.id
  }))
...
? {
 ...
 : { connectOrCreate: prop.user.reviewedWaitlistEntries.map((item) => ({
  ...
  alerts: prop.alerts ? 
  Array.isArray(prop.alerts) && prop.alerts.length > 0 && prop.alerts.every((item: unknown) => typeof item === 'object' && item !== null && ('id' in item || 'symbol' in item) && Object.keys(item).length === 1) ? {
  connect: prop.alerts.map((item) => ({
    id: item.id
  }))
   ...
   */
  async get(props: AlpacaAccountType, globalClient?: ApolloClientType<NormalizedCacheObject>, whereInput?: Record<string, unknown>): Promise<AlpacaAccountType | null> {
    // Maximum number of retries for database connection issues
    const MAX_RETRIES = 3;
    let retryCount = 0;
    let lastError: unknown = null;
   ...
   */
  async findMany(props: AlpacaAccountType, globalClient?: ApolloClientType<NormalizedCacheObject>, whereInput?: Record<string, unknown>): Promise<AlpacaAccountType[] | null> {
    // Maximum number of retries for database connection issues
    const MAX_RETRIES = 3;
    let retryCount = 0;
    let lastError: unknown = null;
