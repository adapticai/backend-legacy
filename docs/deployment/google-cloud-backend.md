...
- **Public hostname:** `api.adaptic.ai` — DNS-flip cutover from Railway.

## Architecture

```
                      ┌────────────────────────┐
                      │ api.adaptic.ai (Cloud  │
                      │   DNS A → Cloud Run    │
                      └───────────┬────────────┘
                                  │  HTTPS / WSS
                                  ▼
...
| Managed SSL cert | `adaptic-backend-cert` | api.adaptic.ai, provisions after DNS cutover |
...
2. Confirm `/readyz` reports `database: connected` through the LB:
   ```bash
   curl -sSL -H 'Host: api.adaptic.ai' http://8.233.34.153/readyz
   ```

Cutover (the moment you flip prod traffic to GCP):

3. **Lower the existing `api.adaptic.ai` DNS TTL** at the registrar
...
6. **Smoke test over HTTPS** once the cert is ACTIVE:
   ```bash
   curl -fsSL https://api.adaptic.ai/livez
   curl -fsSL https://api.adaptic.ai/readyz
   curl -fsSL -X POST https://api.adaptic.ai/graphql \
     -H 'content-type: application/json' -d '{"query":"{__typename}"}'
   ```
7. **Verify end-to-end** from the engine / app / platform sides — these
   already default to `api.adaptic.ai`, so they pick up the new backend
...
If you keep `api.adaptic.ai`, **no env-var changes are required** in any of
`engine/`, `utils/`, `app/`, `platform/`, `backend-legacy/`. All consumers
already default to `api.adaptic.ai`.
...
1. Re-point `api.adaptic.ai` CNAME at `adaptic-backend-stable.up.railway.app`.
